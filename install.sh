...
install_write_env_file() {
  ...
  {
    echo '# Generated by install.sh — re-run install.sh to update PATH'
    printf 'export PATH=%q:$PATH\n' "$bin_dir"
  } >"$env_file"
  echo "install: 已写入 ${env_file} (Wrote ${env_file})"
}

install_remove_legacy_path_blocks() {
  local begin='# >>> ase path >>>'
  local end='# <<< ase path <<<'
  local rc
  while IFS= read -r rc; do
    [[ -n $rc ]] || continue
    install_rc_remove_block "$rc" "$begin" "$end"
  done < <(install_user_rc_files)
}

install_rc_remove_block() {
  local rc_file=$1
  local begin=$2
  local end=$3
  local tmp in_block=0 changed=0 line
  ...
  if (( changed )); then
    mv "$tmp" "$rc_file"
    echo "install: 已移除 ${rc_file} 中的旧 PATH 块 (Removed legacy PATH block from ${rc_file})"
  else
    rm -f "$tmp"
  fi
}
...
install_env_hooks() {
  local bin_dir=$1
  local env_file begin end hook_line rc written=0
  ...
  install_write_env_file "$bin_dir"
  install_remove_legacy_path_blocks
  hook_line=$(printf '[ -f %q ] && . %q' "$env_file" "$env_file")
  while IFS= read -r rc; do
    [[ -n $rc ]] || continue
    install_rc_update_block "$rc" "$begin" "$end" "$hook_line"
    echo "install: 已在 ${rc} 配置 env 加载 (Configured env hook in ${rc})"
    written=1
  done < <(install_env_hook_files)
  if (( written )); then
    echo "install: PATH 由 ${env_file} 统一管理；新开 shell 生效 (PATH managed in ${env_file}; open a new shell)"
  fi
}
